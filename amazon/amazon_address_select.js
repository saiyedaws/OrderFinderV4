bg_port.onMessage.addListener((request) => {
	if (
		request.type === "from_background" &&
		request.command === "paste_address"
	) {
...
function pasteFullName(name) {
	var nameField = document.querySelectorAll(
...
function pasteAddressLine1(addressLine1) {
	var addressLine1Field = document.querySelectorAll(
...
function pastePostalCode(postalCode) {
	var postalCodeField = document.querySelectorAll(
...
function pastePhoneNumber(phoneNumber) {
	var phoneNumberField = document.querySelectorAll(
		'input[id*="enterAddressPhoneNumber"]'
	)[0];
